...
func (s *subscriptions) run(id int) {
	tag := fmt.Sprintf("kcp worker %d ", id)
...
// readable.
func NewServer(l *kcp.Listener) *Server {
	svr := &Server{
		l:             l,
		stats:         &stats{},
...
func (s *Server) newIncomingConn(conn *kcp.UDPSession) *incomingConn {
	return &incomingConn{
		svr:  s,
		conn: conn,
...
// ListenKCP Additional utility function to create a KCP listener
func ListenKCP(addr string) (*kcp.Listener, error) {
